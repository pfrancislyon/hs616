...
This is the correct way to set up a chunk of code to set N equal to 10000 if the #'s were removed to be able run the code in an RMarkdown file using RStudio?
```
#```r
N<- 10000
#```
...
Given the following runtime data, the goal is a 10 times improvement in run time. Which function(s) must improve in what order should be chosen to reach the goal with the most to gain first?
```
             self.time self.pct
...
* Must improve  function_A and function_B, and  function_C 
